ent-BaseSpellbook = книга заклинаний
    .desc = { ent-BaseItem.desc }
ent-WizardsGrimoire = гримуар волшебника
    .suffix = Волшебник
    .desc = Тяжёлый том, содержащий сложные таинственные формулы для подготовки магических заклинаний и колдования сверхъестественного снаряжения. Десятилетия ушли на изучение этой коллекции мистических секретов. Теперь испорьте с их помощью чей-то день.
ent-WizardsGrimoireNoRefund = { ent-WizardsGrimoire }
    .suffix = Волшебник, Без возврата средств
    .desc = { ent-WizardsGrimoire.desc }
ent-WizardsGrimoireDebug = { ent-WizardsGrimoire }
    .desc = Тяжёлый том, содержащий сложные таинственные формулы для подготовки магических заклинаний и колдования сверхъестественного снаряжения. Внутри обложки кто-то написал "ИСПОЛЬЗОВАТЬ ТОЛЬКО ДЛЯ ЭКСПЕРИМЕНТОВ".
    .suffix = Волшебник, DEBUG
ent-SpawnSpellbook = книга заклинания призыв
    .desc = { ent-BaseSpellbook.desc }
ent-ForceWallSpellbook = книга заклинания силовой барьер
    .desc = { ent-BaseSpellbook.desc }
ent-BlinkBook = книга заклинания прыжок
    .desc = { ent-BaseSpellbook.desc }
ent-SmiteBook = книга заклинания кара
    .desc = { ent-BaseSpellbook.desc }
ent-KnockSpellbook = книга заклинания стук
    .desc = { ent-BaseSpellbook.desc }
ent-FireballSpellbook = книга заклинания огненный шар
    .desc = { ent-BaseSpellbook.desc }
ent-ScrollRunes = свиток рун
    .desc = { ent-BaseSpellbook.desc }
